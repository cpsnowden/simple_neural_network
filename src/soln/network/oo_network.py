...
class Neuron(VoltageHolder):
    ...
    def conduct_forward(self):
        # z = wT a + b
        self.voltage = reduce(add, map(lambda x: x.conduct(), self.input_edge)) + self.bias
        self._activation = Neuron.sigmoid(self.voltage) if self._is_hidden else self.voltage
    ...
    @staticmethod
    def sigmoid(voltage):
        return 1.0 / (1.0 + np.exp(-voltage))

    @staticmethod
    def sigmoid_prime(voltage):
        return Neuron.sigmoid(voltage) * (1 - Neuron.sigmoid(voltage))
...
class NN(object):
    def __init__(self, *layer_sizes):
        ...
        for index in xrange(len(layer_sizes) - 1):
            left_neuron_layer = self.neuron_layers[index]
            right_neuron_layer = self.neuron_layers[index + 1]
    ...
    def backwards_propagate(self, expected_outputs):
        ...
        for layer_n_r, layer in enumerate(reversed(self.neuron_layers)):
            for neuron in layer:
                NN.take_error(neuron)
                if layer_n_r < self.number_of_layers - 1:
                    NN.propagate_error_back(neuron)
    ...
    def take_error(neuron):
        neuron.error = reduce(add, map(lambda x: x.error, neuron.output_edge)) * Neuron.sigmoid_prime(neuron.voltage)
...
if __name__ == "__main__":
    net = NN(2, 3, 2)
    net.train_from_batches_fast(None, 3, None)
